...
class CursosController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $tableData = Cursos::when(!empty($ciclos), function ($query) use ($ciclos) {
            return $query->whereIn('ciclo', $ciclos);
        })->orderBy("id")->get();
        $ciclos_all = Ciclos::all()->pluck("nombre", "id");

        if ($request->isMethod('post')) {
            return redirect()->route('admin.cursos.index');
        }

        return view("admin.cursos.index")->with(compact('admiko_data', "tableData",'ciclos_all','ciclos'));
    }
