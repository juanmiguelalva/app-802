...
@section('breadcrumbs')
<li class="breadcrumb-item active" aria-current="page">SECIGRISTAS</li>
@endsection
@section('pageTitle')
<h1>SECIGRISTAS</h1>
@endsection
...
@endsection
@section('content')
<div class="card secigristas_index admikoIndex">
    <div class="card-body">
        <div class="tableBox" id="tableBox">
            ...
            <div class="tableLayout pb-2">
                    ...
                    <thead>
                        <tr data-sort-method='thead'>
							...
							<th scope="col" class="d-none d-sm-table-cell">Unidad Receptora</th>
                            <th scope="col" class="w-5 no-sort" data-orderable="false">{{trans("admiko.table_edit")}}</th>
                            @if(Gate::allows('secigristas_allow'))
                    ...
                    @foreach($tableData as $data)
                        <tr>
							<td class="w-5"><a href="{{route("admin.secigristas.edit",[$data->id])}}">{{$data->id}}</a></td>
							<td>{{$data->apellidos}} {{$data->nombres}}</td>
							<td class="d-none d-sm-table-cell">{!!Str::limit(strip_tags($data["unidad_receptora"]), 50, "...")!!}</td>
                            <td class="w-5 no-sort"><a href="{{route("admin.secigristas.edit",[$data->id])}}"><i class="fas fa-edit fa-fw"></i></a></td>
                            @if(Gate::allows(['secigristas_allow']))
            ...
            <div class="row">
                <div class="col-12 col-sm order-3 order-sm-0 pt-2">
                    @if(Gate::any(['secigristas_allow']))
                        <a href="{{route('admin.secigristas.create')}}" class="btn btn-primary" role="button"><i class="fas fa-plus fa-fw"></i> {{trans('admiko.table_add')}}</a>
                    @endIf
                </div>
    ...
    <div class="modal fade" id="deleteConfirm" tabindex="-1" role="dialog" aria-labelledby="deleteConfirm" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form method="post" class="w-100" action="{{route("admin.secigristas.delete")}}">
            @method('DELETE')
            @csrf
