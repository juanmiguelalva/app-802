...
@if(Gate::any(['cursos_allow','cursos_edit','docentes_allow','docentes_edit','distribucion_de_aulas_allow','distribucion_de_aulas_edit','tickets_allow','tickets_edit','horario_allow']))
<li class="nav-item dropdown{{ $admiko_data['sideBarActiveFolder'] === "_principal1" ? " open" : "" }}">
    <a href="#" class="nav-link dropdown-link">PRINCIPAL</a>
	...
	@endIf
	@if(Gate::any(['distribucion_de_aulas_allow', 'distribucion_de_aulas_edit']))
		<li class="nav-item{{ $admiko_data['sideBarActive'] === "distribucion_de_aulas" ? " active" : "" }}"><a class="nav-link dropdown-item" href="{{route('admin.distribucion_de_aulas.index')}}"><i class="fas fa-file-alt fa-fw"></i>Distribución de Aulas</a></li>
	@endIf
	...
	@endIf
	@if(Gate::any(['horarios_allow']))
		<li class="nav-item{{ $admiko_data['sideBarActive'] === "horarios" ? " active" : "" }}"><a class="nav-link" href="{{route('admin.horarios.index')}}"><i class="fas fa-calendar-alt fa-fw"></i>Horarios</a></li>
	@endIf
...
<li class="nav-item dropdown{{ $admiko_data['sideBarActiveFolder'] === "_sitio_web" ? " open" : "" }}">
    <a href="#" class="nav-link dropdown-link">SITIO WEB</a>
...
@endIf
@if(Gate::any(['cargo_allow','cargo_edit','tipo_profesor_allow','tipo_profesor_edit','ciclos_allow','ciclos_edit','condicion_allow','condicion_edit','especialidades_allow','especialidades_edit','grados_allow','grados_edit','aulas_allow','aulas_edit','secciones_allow','secciones_edit','tipo_publicacion_allow','tipo_publicacion_edit','unidad_allow','unidad_edit','dias_allow','dias_edit']))
<li class="nav-item dropdown{{ $admiko_data['sideBarActiveFolder'] === "_recursos" ? " open" : "" }}">
    <a href="#" class="nav-link dropdown-link">RECURSOS</a>
