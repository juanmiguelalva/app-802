...
@section('breadcrumbs')
<li class="breadcrumb-item active" aria-current="page">Distribución de Aulas</li>
@endsection
@section('pageTitle')
...
<div class="card distribucion_de_aulas_index admikoIndex">
            ...
            <div class="tableLayout pb-2">
                    ...
                    <thead>
                        <tr data-sort-method='thead'>
							...
							<th scope="col" class="text-nowrap">Ciclo</th>
							<th scope="col" class="text-nowrap d-none d-sm-table-cell">Aula</th>
							<th scope="col" class="text-nowrap d-none d-md-table-cell">Sección</th>
                            <th scope="col" class="text-nowrap d-none d-md-table-cell">Turno</th>
                    ...
                    @foreach($tableData as $data)
                        <tr>
							...
							<td class="text-nowrap">{{$data->ciclo_id->nombre??""}}</td>
							<td class="text-nowrap d-none d-sm-table-cell">{{$data->aula_id->codigo??""}}</td>
							<td class="text-nowrap d-none d-md-table-cell">{{$data->seccin_id->nombre??""}}</td>
                            <td class="text-nowrap d-none d-md-table-cell">@if($data->turno == 'M') Mañana @else Noche @endif</td>
            ...
            <div class="row">
                <div class="col-12 col-sm order-3 order-sm-0 pt-2">
                    @if(Gate::any(['distribucion_de_aulas_allow']))
                        <a href="{{route('admin.distribucion_de_aulas.create')}}" class="btn btn-primary" role="button"><i class="fas fa-plus fa-fw"></i> {{trans('admiko.table_add')}}</a>
                    @endIf
                </div>
    ...
    <div class="modal fade" id="deleteConfirm" tabindex="-1" role="dialog" aria-labelledby="deleteConfirm" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form method="post" class="w-100" action="{{route("admin.distribucion_de_aulas.delete")}}">
            @method('DELETE')
            @csrf
