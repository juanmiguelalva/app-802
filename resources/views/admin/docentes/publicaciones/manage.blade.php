    ...
    <form method="POST" action="{{ $admiko_data['formAction'] }}" enctype="multipart/form-data" class="needs-validation" novalidate>
        ...
        <div class="card-body">
            ...
            <div class="row">
				...
				@endif
				@if(!isset($data) || $data->admiko_dynamic_fields->contains("tipo"))
                <div class=" col-12">
                    <div class="form-group row">
                        ...
                        <div class="col-md-10">
                            <select class="form-select" id="tipo" name="tipo" required="true">
                                ...
                                @foreach($tipo_publicacion_all as $id => $value)
                                    <option value="{{ $id }}" {{ (old('tipo') ? old('tipo') : $data->tipo ?? '') == $id ? 'selected' : '' }}>{{ $value }}</option>
                                @endforeach
                            </select>
				...
				@endif
				@if(!isset($data) || $data->admiko_dynamic_fields->contains("enlace"))
                <div class=" col-12">
                    <div class="form-group row">
                        <label for="enlace" class="col-md-2 col-form-label">Enlace:</label>
                        <div class="col-md-10">
                            <input type="text" class="form-control" id="enlace" name="enlace"  placeholder="Enlace"  value="{{{ old('enlace', isset($data)?$data->enlace : '') }}}">
                            <div class="invalid-feedback @if ($errors->has('enlace')) d-block @endif">{{trans('admiko.required_text')}}</div>
                            <small id="enlace_help" class="text-muted"></small>
                        </div>
                    </div>
                </div>
				@endif
        ...
        <div class="card formPage">
            ...
            <form method="POST" action="{{route('admin.publicaciones.admiko_dynamic_fields',[Request()->admiko_docentes_id,$data->id])}}">
                ...
                <div class=" col-12">
                    <div class="form-group row">
                        <label for="admiko_dynamic_field_tipo" class="col-md-2 col-form-label">Tipo:</label>
                        <div class="col-md-10">
                            <div class="form-check form-checkbox">
                                <input class="form-check-input" type="checkbox" id="admiko_dynamic_field_tipo" name="admiko_dynamic_fields[]" @if($data->admiko_dynamic_fields->contains("tipo")) checked @endif value="tipo">
                                <label class="form-check-label" for="admiko_dynamic_field_tipo"></label>
                            </div>
                        </div>
                    </div>
                </div>
                <div class=" col-12">
                    <div class="form-group row">
                        <label for="admiko_dynamic_field_enlace" class="col-md-2 col-form-label">Enlace:</label>
                        <div class="col-md-10">
                            <div class="form-check form-checkbox">
                                <input class="form-check-input" type="checkbox" id="admiko_dynamic_field_enlace" name="admiko_dynamic_fields[]" @if($data->admiko_dynamic_fields->contains("enlace")) checked @endif value="enlace">
                                <label class="form-check-label" for="admiko_dynamic_field_enlace"></label>
                            </div>
                        </div>
                    </div>
                </div>
