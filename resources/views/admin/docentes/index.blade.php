            ...
            <div class="tableLayout pb-2 table-responsive">
                    ...
                    <thead>
                        <tr data-sort-method='thead'>
							...
							<th scope="col" class="text-nowrap d-none d-md-table-cell">Grado</th>
							<th scope="col" class="">Apellidos y Nombres</th>
                    ...
                    @foreach($tableData as $data)
                        <tr class="align-middle">
							<td class="w-5 d-none d-md-table-cell"><a href="{{route("admin.docentes.edit",[$data->id])}}">{{$data->id}}</a></td>
                            ...
                            <a href="{{ Storage::disk(config("admiko_config.filesystem"))->url($admiko_data["fileInfo"]["foto"]["original"]["folder"].$data->foto) }}" target="_blank" class="tableImage">
                                <img class="rounded-circle" width="55px" height="55px" src="{{ Storage::disk(config("admiko_config.filesystem"))->url($admiko_data["fileInfo"]["foto"]["original"]["folder"].$data->foto) }}">
                            </a>
                            @else
                            @php
                                $iniciales = strtoupper(substr($data->apellidos, 0, 1) . substr($data->nombres, 0, 1));
                            @endphp
                            <style>
                                .iniciales{
                            ...
                            <div class="m-0 iniciales rounded-circle d-flex justify-content-center align-items-center h5">{{ $iniciales }}</div>
                            @endIf</td>
                            <td class="d-none d-sm-table-cell">{{$data->grado_id->abreviatura??""}}</td>
							<td class="">{{$data->apellidos}} {{$data->nombres}}
                                {{-- <br>{{$data->correo}} --}}
                            </td>
							...
							<td class="text-nowrap text-center childPageLink">
                                <a href="{{route("admin.publicaciones.index",[$data->id])}}"><i class="fas fa-bookmark fa-fw"></i></a>
                            </td>
