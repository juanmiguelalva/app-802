...
@endsection
@section('content')
<div class="card autoridades_index admikoIndex">
    <div class="card-body">
        <div class="tableBox" id="tableBox">
            ...
            <div class="tableLayout pb-2">
                    ...
                    <thead>
                        <tr data-sort-method='thead'>
							...
							<th scope="col" class="text-nowrap d-none d-md-table-cell">Cargo</th>
							<th scope="col" class="text-nowrap d-none d-lg-table-cell">Unidad</th>
                            <th scope="col" class="w-5 no-sort" data-orderable="false">{{trans("admiko.table_edit")}}</th>
                            @if(Gate::allows('autoridades_allow'))
                    ...
                    @foreach($tableData as $data)
                        <tr>
							<td class="w-5"><a href="{{route("admin.autoridades.edit",[$data->id])}}">{{$data->id}}</a></td>
							<td>@if (isset($data->foto) && Storage::disk(config("admiko_config.filesystem"))->exists($admiko_data["fileInfo"]["foto"]["original"]["folder"].$data->foto))
							...
							<td>{{$data->grado_id->abreviatura??""}} {{$data->apellidos}} {{$data->nombres}}</td>
							<td class="text-nowrap d-none d-md-table-cell">{{$data->cargo_id->nombre??""}}</td>
							<td class="text-nowrap d-none d-lg-table-cell">{{$data->unidad_id->nombre??""}}</td>
                            <td class="w-5 no-sort"><a href="{{route("admin.autoridades.edit",[$data->id])}}"><i class="fas fa-edit fa-fw"></i></a></td>
                            @if(Gate::allows(['autoridades_allow']))
            ...
            <div class="row">
                <div class="col-12 col-sm order-3 order-sm-0 pt-2">
                    @if(Gate::any(['autoridades_allow']))
                        <a href="{{route('admin.autoridades.create')}}" class="btn btn-primary" role="button"><i class="fas fa-plus fa-fw"></i> {{trans('admiko.table_add')}}</a>
                    @endIf
                </div>
    ...
    <div class="modal fade" id="deleteConfirm" tabindex="-1" role="dialog" aria-labelledby="deleteConfirm" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form method="post" class="w-100" action="{{route("admin.autoridades.delete")}}">
            @method('DELETE')
            @csrf
