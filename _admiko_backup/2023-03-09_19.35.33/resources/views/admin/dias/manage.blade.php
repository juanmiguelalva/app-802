            ...
            <div class="row">
                ...
                <div class=" col-12">
                    <div class="form-group row multiSelect">
                        <label for="p" class="col-md-2 col-form-label">p:</label>
                        <div class="col-md-10" style="position: relative">
                            <select name="p[]" multiple="multiple" data-placeholder="{{trans('admiko.select_from_list')}}"  id="p"  style="width: 100%" data-allow-clear="true">
                            @if(isset($data))
                                @php $p_select = $data->p_many_select(); @endphp
                            @endIf
                            @foreach($p_all as $id => $value)
                                @php $selected = ""; @endphp
                                @if(in_array($id, old('p', [])))
                                    @php $selected = "selected"; @endphp
                                @elseIf(isset($data) && $p_select->contains($id))
                                    @php $selected = "selected"; @endphp
                                @endIf
                                <option value="{{ $id }}" {{$selected}}>{{ $value }}</option>
                            @endforeach
                            </select>
                            <div class="invalid-feedback @if ($errors->has('p')) d-block @endif">{{trans('admiko.required_text')}}</div>
                            <small id="p_help" class="text-muted"></small>
                        </div>
                    </div>
                </div>
            </div>
