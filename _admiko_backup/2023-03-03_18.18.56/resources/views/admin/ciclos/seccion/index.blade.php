...
@section('breadcrumbs')
<li class="breadcrumb-item active"><a href="{{ route("admin.ciclos.index") }}">Ciclos</a></li>
...
@endsection
@section('backBtn')
<a href="{{route("admin.ciclos.index")}}"><i class="fas fa-angle-left"></i> {{trans('admiko.page_back_btn')}}</a>
@endsection
@section('content')
<div class="card seccion_index admikoIndex">
    <div class="card-body">
        <div class="tableBox" id="tableBox">
            ...
            <div class="tableLayout pb-2">
                    ...
                    @foreach($tableData as $data)
                        <tr>
							<td class="w-5"><a href="{{route("admin.seccion.edit",[Request()->admiko_ciclos_id,$data->id])}}">{{$data->id}}</a></td>
							<td class="text-nowrap">{{$data->nombre}}</td>
                            <td class="w-5 no-sort"><a href="{{route("admin.seccion.edit",[Request()->admiko_ciclos_id,$data->id])}}"><i class="fas fa-edit fa-fw"></i></a></td>
                            @if(Gate::allows(['seccion_allow']))
            ...
            <div class="row">
                <div class="col-12 col-sm order-3 order-sm-0 pt-2">
                    @if(Gate::any(['seccion_allow']))
                        <a href="{{route('admin.seccion.create',[Request()->admiko_ciclos_id])}}" class="btn btn-primary" role="button"><i class="fas fa-plus fa-fw"></i> {{trans('admiko.table_add')}}</a>
                    @endIf
                </div>
    ...
    <div class="modal fade" id="deleteConfirm" tabindex="-1" role="dialog" aria-labelledby="deleteConfirm" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form method="post" class="w-100" action="{{route("admin.seccion.delete",[Request()->admiko_ciclos_id])}}">
            @method('DELETE')
            @csrf
